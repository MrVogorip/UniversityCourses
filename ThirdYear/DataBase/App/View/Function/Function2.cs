...
namespace HomeWorkDataBase.View
{
    public partial class Function2 : Form
    {
        ...
        private void buttonPoolMaster_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = Logon.DB.ExecuteQuery($"SELECT dbo.PoolMaster('{startDate}','{endDate}','{fullNameMaster}') AS result");
                if (result.Count == 0)
                {
